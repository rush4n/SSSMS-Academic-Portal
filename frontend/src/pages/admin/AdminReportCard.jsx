...
const AdminReportCard = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const profileRes = await api.get(`/admin/student/${studentId}/profile`);
            setStudentInfo(profileRes.data);

            // Fetch report card
            const reportRes = await api.get(`/admin/report-card/${studentId}`);
            setReport(reportRes.data);
        } catch (err) {
            setError(err.response?.data || 'Failed to fetch report card.');
            setReport(null);
            setStudentInfo(null);
        } finally {
            setLoading(false);
